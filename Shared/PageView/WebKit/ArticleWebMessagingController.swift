...
class ArticleWebMessagingController: NSObject {
    ...
    func setup(with webView: WKWebView, language: String, theme: Theme, layoutMargins: UIEdgeInsets, leadImageHeight: CGFloat = 0, areTablesInitiallyExpanded: Bool = false, textSizeAdjustment: Int? = nil, userGroups: [String] = []) throws {
    ...
    func addFooter(articleURL: URL, restAPIBaseURL: URL, menuItems: [PageContentService.Footer.Menu.Item], lastModified: Date?) {
        guard let title = articleURL.lastPathComponent.addingPercentEncoding(withAllowedCharacters: .encodeURIComponentAllowed) else {
            return
        }
        ...
        webView?.evaluateJavaScript("pcs.c1.Footer.add(\(parametersJS))", completionHandler: { (result, error) in
            if let error = error {
                Logger.standard.error("Error adding footer: \(error as NSObject)")
            }
        })
    }
    ...
    var currentMargins: UIEdgeInsets = .zero
    func updateMargins(with layoutMargins: UIEdgeInsets, leadImageHeight: CGFloat) {
        guard layoutMargins != currentMargins else {
            return
        }
        currentMargins = layoutMargins
    ...
    func prepareForScroll(to anchor: String, highlight: Bool, completion: @escaping (Result<Void, Error>) -> Void) {
        ...
        webView.evaluateJavaScript("pcs.c1.Page.prepareForScrollToAnchor(`\(anchor.sanitizedForJavaScriptTemplateLiterals)`, {highlight: \(highlight ? "true" : "false")})") { (result, error) in
            if let error = error {
                Logger.standard.error("Error attempting to scroll to anchor: \(anchor) \(error as NSObject)")
